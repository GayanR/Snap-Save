...
/**
 * Created by Nuwan on 6/24/2015.
 */
public class BusBillImageProcess {
    ...
    public static String[][] busImageOcrForFiltering(String path) {
        ...
        Mat grad = new Mat();
        Mat morphKernel = Imgproc.getStructuringElement(Imgproc.MORPH_ELLIPSE, new Size(500, 4));//3,3
        ...
        morphKernel = Imgproc.getStructuringElement(Imgproc.MORPH_RECT, new Size(500, 4)); //9,1
...
//                Imgproc.adaptiveThreshold(subContourRegion, subContourRegion, 255, Imgproc.ADAPTIVE_THRESH_GAUSSIAN_C, Imgproc.THRESH_BINARY, 75, 17);
    ...
    public static List<Mat> busImageOcrForDisplaying(String path) {
        if (!OpenCVLoader.initDebug()) {
            Log.i(LOGTAG, "OPENCV CRASHED");
...
//                Imgproc.adaptiveThreshold(contourRegion, contourRegion, 255, Imgproc.ADAPTIVE_THRESH_GAUSSIAN_C, Imgproc.THRESH_BINARY, 75, 17);
...
//        }
        ...
        if (recognizedText.contains("~")) {
            ...
            recognizedText = recognizedText.replaceAll("[~]+", "-");
            Log.v(LOGTAG, "NEW OCRED TEXT: " + recognizedText);

        }
        else if(recognizedText.contains("l")||recognizedText.contains("I")) {
            recognizedText = recognizedText.replaceAll("[lI]+", "1");
            Log.v(LOGTAG, "NEW OCRED TEXT: " + recognizedText);

        }
        else if(recognizedText.contains("Z")||recognizedText.contains("z")) {
            recognizedText = recognizedText.replaceAll("[Zz]+", "2");
            Log.v(LOGTAG, "NEW OCRED TEXT: " + recognizedText);
